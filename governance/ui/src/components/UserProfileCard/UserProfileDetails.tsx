...
<>
    ...
    <Flex flexDir="column" w="100%" ml="2">
      <Flex justifyContent="space-between">
        <Text
          ...
          overflow="hidden"
        >
          {Members?.username ? Members.username : prettyString(Members?.address || '')}
        </Text>
      </Flex>
      <Text
        fontSize="12px"
  ...
  <Flex mb="4" gap="3">
    <Socials discord={Members?.discord} github={Members?.github} twitter={Members?.twitter} />
    ...
    >
      ...
      <Tooltip label={walletToolTipLabel} isOpen={isWalletTooltipOpen}>
        <CopyIcon
          ...
          onClick={() => {
            navigator.clipboard.writeText(Members?.address || '');
  ...
  <Flex mt="auto" gap="2" flexDir="column">
    ...
    {councilPeriod === '2' && (
      <>
        ...
        <Button
          variant={
            !isNominated && isOwn ? 'outline' : !isAlreadyVoted && !isSelected ? 'solid' : 'outline'
          }
          colorScheme={
            ...
            if (isMotherchain(parsedNetwork) && !(process.env.CI === 'true')) {
              ...
              if (isAlreadyVoted) {
                ...
                dispatch({
                  type: activeCouncil.toUpperCase(),
                  payload: {
                    action: Members?.address.toLowerCase(),
                    network: parsedNetwork,
                    epochId: epochId?.toString(),
                    wallet: activeWallet?.address,
                  },
        ...
        >
          <Text maxW="250px" textOverflow="ellipsis" whiteSpace="nowrap" overflow="hidden">
            ...
            {Members?.ens || Members?.username ? Members.username : prettyString(Members!.address)}
          </Text>
        </Button>
      </>
    )}
